...
const Dashboard: React.FC<DashboardProps> = ({ isDarkMode = false }) => {
  ...
  const userMetric = { label: 'Total Users', value: '1,245', change: 12, icon: 'ðŸ‘¥', color: 'text-blue-600' };
  const ordersMetric = { label: 'Total Orders', value: '8,920', change: 8, icon: 'ðŸ“¦', color: 'text-green-600' };
  const revenueMetric = { label: 'Revenue', value: '$89,230', change: 15, icon: 'ðŸ’°', color: 'text-orange-600' };
  const sessionsMetric = { label: 'Active Sessions', value: '342', change: -3, icon: 'ðŸŸ¢', color: 'text-green-600' };

  const orderStatuses = [
    { label: 'Completed', value: 320, color: 'bg-green-500', icon: 'âœ“' },
    { label: 'In Progress', value: 145, color: 'bg-blue-500', icon: 'âŸ³' },
    { label: 'Pending', value: 78, color: 'bg-yellow-500', icon: 'â³' },
    { label: 'Failed', value: 12, color: 'bg-red-500', icon: 'âœ•' },
  ];

  const recentActivities = [
    { id: '1', action: 'New user registration', timestamp: '2 mins ago', user: 'System', icon: 'ðŸ“' },
    { id: '2', action: 'Order #12345 completed', timestamp: '15 mins ago', user: 'Order Service', icon: 'ðŸ“¦' },
    { id: '3', action: 'Revenue report generated', timestamp: '1 hour ago', user: 'Reporting', icon: 'ðŸ“Š' },
    { id: '4', action: 'System backup completed', timestamp: '2 hours ago', user: 'System', icon: 'ðŸ’¾' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              >
                {activeLayout.isEditing ? 'âœ“ Done Editing' : 'âœŽ Edit Layout'}
              </Button>
            </div>
          </div>
        </div>
